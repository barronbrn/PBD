        ...
        <form action="{{ route('transaksi.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="kode_obat" name="kode_obat" required>
                    ...
                    @foreach($obat as $o)
                        <option value="{{ $o->kode }}" {{ old('kode_obat') == $o->kode ? 'selected' : '' }}>
                            {{ $o->kode }} - {{ $o->nama_obat }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="nilai_fisik" name="nilai_fisik" 
                    value="{{ old('nilai_fisik') }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="selisih_nilai" name="selisih_nilai" 
                    value="{{ old('selisih_nilai') }}" required>
            </div>
